...
import numpy as np
import pandas as pdsample_rate
import xarray as xr
import datetime
...
def find_hydrocat_headerend(hydrocat_lines):
...
def find_hydrocat_instrument_start_time(hydrocat_lines):
...
def find_hydrocat_sample_rate(hydrocat_lines):
...
def read_hydrocat_seabirdcnv(hydrocat_file):
...
def read_hydrocat_nemo_lineparsing(hydrocat_file):
    ...
    hdr_end_ind = find_hydrocat_headerend(lines)
    startind = hdr_end_ind + 1
    instrument_start_time = find_hydrocat_instrument_start_time(lines)
    ...
    sample_rate = find_hydrocat_sample_rate(lines)
    ...
    hcat_rawdf = pd.DataFrame(data = list(zip(timestamp, record, 
...
def _load_hydrocat_deployment_window(nemo_name, deployment_name):
...
def _find_hydrocat_files(datadir, nemo_name, deployment_name, serial_num=None, status=None):
    """Discover HydroCat data files in datadir matching deployment and optional serial."""
    ...
    for fname in os.listdir(datadir):
        ...
        if not flower.endswith(required_suffix):
            continue
        if not ('hydrocat' in flower or 'hcat' in flower or 'ctd' in flower):
            continue
...
def nemo_hydrocat_wrapper(nemo_name, deployment_name, instrument_name='hydrocat', serial_num=None):
    
    lat, lon, depth, start_deployment, end_deployment, status = _load_hydrocat_deployment_window(
        nemo_name,
        deployment_name
    )
        ...
        if isinstance(instruments, dict):
            for _, instrument_record in instruments.items():
                ...
                if serial_value is None:
                    continue
                if ('hydrocat' not in model_name) and ('hcat' not in model_name):
                    continue

                serial_text = str(serial_value).strip()
                if serial_text == '':
                    continue
                if serial_text not in hydrocat_serials:
                    hydrocat_serials.append(serial_text)

        if len(hydrocat_serials) == 0:
            print(
                f"No HydroCat serial numbers were found in deployment info for "
                f"{nemo_name} - {deployment_name}."
            )
            return

        print(
            f"No serial number provided. Found {len(hydrocat_serials)} HydroCat serial(s): "
            f"{', '.join(hydrocat_serials)}"
        )
        for current_serial in hydrocat_serials:
            print(f"\nProcessing HydroCat serial {current_serial}...")
            nemo_hydrocat_wrapper(nemo_name, deployment_name, instrument_name, current_serial)
        return

    # Discover matching data files
    hcat_files = _find_hydrocat_files(datadir, nemo_name, deployment_name, serial_num, status)
    if len(hcat_files) == 0:
        print(f'   No HydroCat files found in {datadir}')
        return
    print(f'   Found {len(hcat_files)} HydroCat file(s): {hcat_files}')
    ...
    frames = []
    for fname in hcat_files:
        fpath = os.path.join(datadir, fname)
        if status == 'recovered':
            df, sample_rate = read_hydrocat_seabirdcnv(fpath)
            frames.append(df)
        else:
            try:
                df, sample_rate = read_hydrocat_nemo_dataframe(fpath)
                frames.append(df)
            except Exception:
                print(f'   Failed dataframe parsing for {fname}; trying line parsing.')
                df, sample_rate = read_hydrocat_nemo_lineparsing(fpath)
                frames.append(df)

    if len(frames) == 1:
        hcat_rawdf = frames[0].copy()
    else:
        hcat_rawdf = pd.concat(frames, ignore_index=True)
    ...
    hcat_std = pd.DataFrame()
    instrument_info = {}


    if status == 'recovered':
        # Find the matching HydroCat instrument for this serial number when possible.
        if serial_num is not None:
            ...
            if instrument_info is None:
                print(
                    f"   No matching deployment instrument found for HydroCat serial {serial_num}. "
        ...
        # read_hydrocat_seabirdcnv provides timestamp seconds and derived seawater variables.
        if 'timestamp' in hcat_rawdf.columns:
            sample_time = pd.to_datetime(hcat_rawdf['timestamp'], unit='s', origin='unix', utc=True).dt.tz_localize(None)
        elif 'date' in hcat_rawdf.columns:
            sample_time = pd.to_datetime(hcat_rawdf['date'])
            if getattr(sample_time.dt, 'tz', None) is not None:
                sample_time = sample_time.dt.tz_localize(None)
        else:
            raise KeyError('Recovered HydroCat dataframe missing timestamp/date columns.')
        ...
        hcat_std['sample_time'] = sample_time
        hcat_std['instrument_timestamp'] = sample_time
        hcat_std['record_number'] = np.arange(len(hcat_rawdf))
        hcat_std['sea_water_temperature'] = hcat_rawdf['sea_water_temperature'].to_numpy()
        hcat_std['sea_water_electrical_conductivity'] = hcat_rawdf['sea_water_electrical_conductivity'].to_numpy()
        hcat_std['sea_water_pressure'] = hcat_rawdf['sea_water_pressure'].to_numpy()
        hcat_std['sea_water_practical_salinity'] = hcat_rawdf['sea_water_practical_salinity'].to_numpy()
        hcat_std['sea_water_sigma_theta'] = hcat_rawdf['sea_water_sigma_theta'].to_numpy()
        hcat_std['speed_of_sound_in_sea_water'] = hcat_rawdf['speed_of_sound_in_sea_water'].to_numpy()
        hcat_std['mass_concentration_of_oxygen_in_sea_water'] = hcat_rawdf['mass_concentration_of_oxygen_in_sea_water'].to_numpy()
        hcat_std['fractional_saturation_of_oxygen_in_sea_water'] = hcat_rawdf['fractional_saturation_of_oxygen_in_sea_water'].to_numpy()
    else:
        # Realtime parsing output is converted to seabird-CNV naming conventions.
        hcat_std['sample_time'] = pd.to_datetime(hcat_rawdf['Timestamp'])
        hcat_std['instrument_timestamp'] = pd.to_datetime(hcat_rawdf['Instrument_Timestamp'])
        hcat_std['record_number'] = hcat_rawdf['RecordNumber'].to_numpy()
        hcat_std['sea_water_temperature'] = hcat_rawdf['Temperature_degC'].to_numpy()
        hcat_std['sea_water_electrical_conductivity'] = hcat_rawdf['Conductivity_S_m'].to_numpy()
        hcat_std['sea_water_pressure'] = hcat_rawdf['Pressure_dbar'].to_numpy()



        sal_input = hcat_rawdf['sea_water_practical_salinity'].to_numpy()
        sal_calc = gsw.SP_from_C(
            np.array([10 * ii for ii in hcat_std['sea_water_electrical_conductivity']]),
            hcat_std['sea_water_temperature'].to_numpy(),
            hcat_std['sea_water_pressure'].to_numpy()
        )
        hcat_std['sea_water_practical_salinity'] = np.array([
            sal_calc[ii] if pd.isna(sal_input[ii]) else sal_input[ii]
            for ii in range(0, len(sal_input))
        ])

        sal_SA = gsw.SA_from_SP(
            hcat_std['sea_water_practical_salinity'].to_numpy(),
            hcat_std['sea_water_pressure'].to_numpy(),
            lon,
            lat
        )
        ...
        hcat_std['sea_water_sigma_theta'] = gsw.sigma0(sal_SA, temp_CT)
        hcat_std['speed_of_sound_in_sea_water'] = gsw.sound_speed(sal_SA, temp_CT, hcat_std['sea_water_pressure'].to_numpy())
        ...
        O2_sol_mg_L = (
            ...
            * (32 / 1000) * (1000 + hcat_std['sea_water_sigma_theta'].to_numpy()) * (1 / 1000)
        )
        hcat_std['fractional_saturation_of_oxygen_in_sea_water'] = (
            100 * hcat_std['mass_concentration_of_oxygen_in_sea_water'].to_numpy() / O2_sol_mg_L
        )

    # Keep data only within the deployment window.
    valid_time_mask = ~hcat_std['sample_time'].isna()
    if start_deployment is not None:
        valid_time_mask = valid_time_mask & (hcat_std['sample_time'] >= start_deployment)
    if end_deployment is not None:
        valid_time_mask = valid_time_mask & (hcat_std['sample_time'] <= end_deployment)
    hcat_std = hcat_std[valid_time_mask]

    if len(hcat_std) == 0:
        print('   No new HydroCat data. Continue on.')
        return

    sample_rate = _estimate_sample_rate_seconds(hcat_std['sample_time'])

    instrument_depth = instrument_info.get('instrument_depth', depth)
    hcat_std['depth'] = [instrument_depth for _ in range(len(hcat_std))]
    
    

    # Repackage standardized data into legacy netCDF-writing schema.
    hcat_std = hcat_std.reset_index(drop=True)
    n_samples = len(hcat_std)
    hcat_df = pd.DataFrame({
        'buoyname': [nemo_name for _ in range(0, n_samples)],
        'time': hcat_std['sample_time'].to_numpy().squeeze(),
        'instrument_timestamp': hcat_std['instrument_timestamp'].to_numpy().squeeze(),
        'record_number': hcat_std['record_number'].to_numpy().squeeze(),
        'sea_water_pressure': hcat_std['sea_water_pressure'].to_numpy().squeeze(),
        'depth': hcat_std['depth'].to_numpy().squeeze(),
        'sea_water_temperature': hcat_std['sea_water_temperature'].to_numpy().squeeze(),
        'sea_water_electrical_conductivity': hcat_std['sea_water_electrical_conductivity'].to_numpy().squeeze(),
        'sea_water_practical_salinity': hcat_std['sea_water_practical_salinity'].to_numpy().squeeze(),
        'sea_water_sigma_theta': hcat_std['sea_water_sigma_theta'].to_numpy().squeeze(),
        'mass_concentration_of_oxygen_in_sea_water': hcat_std['mass_concentration_of_oxygen_in_sea_water'].to_numpy().squeeze(),
        'fractional_saturation_of_oxygen_in_sea_water': hcat_std['fractional_saturation_of_oxygen_in_sea_water'].to_numpy().squeeze(),
        'speed_of_sound_in_sea_water': hcat_std['speed_of_sound_in_sea_water'].to_numpy().squeeze()
    })

    # Add empty variable columns for future use
    hcat_df['sea_water_ph_reported_on_total_scale'] = [np.nan for _ in range(0, n_samples)]
    hcat_df['mass_concentration_of_chlorophyll_a_in_sea_water'] = [np.nan for _ in range(0, n_samples)]
    hcat_df['sea_water_turbidity'] = [np.nan for _ in range(0, n_samples)]
    ...
    hcat_qcdf = nemo_qc.process_qartod_tests(
        hcat_df,
    ...
    for var in ['sea_water_pressure', 'sea_water_temperature', 
        ...
        hcat_df[var] = [np.round(ii,6) if not pd.isna(ii) else np.nan for ii in hcat_df[var].to_numpy()]

    # Replace any NaN values with -555 for netCDF compatibility
    hcat_df.fillna(-555, inplace=True)
    
    ########################################
    # Make the lander netCDF
    nemo_funcs.make_ctd_netCDF(nemo_info, savedir, hcat_df, hcat_qcdf)
...
def main():
    ...
    nemo_hydrocat_wrapper(args.buoy, args.deployment, args.instrument, args.serial)
